...
function getReportFilter($searchWord,$mode) {
	...
	if ($mode != '' && $mode != 'All' ) {  
		...
		if($mode == 'Leads'){			
			...
			$opr2 = "NotEqual"; 
			$row++;
			$CriteriaRow1 .= "Criteria Row=\"$row\" Field=\"visitDate\" Operator=\"$opr2\" Value=\"\" <br>";
			$CriteriaRow .= "<Criteria Row=\"$row\" Field=\"visitDate\" Operator=\"$opr2\" Value=\"\" />";
			...
			$opr1 = "NotEqual"; 
			$row++;
			$CriteriaRow1 .= "Criteria Row=\"$row\" Field=\"Appointment_Date\" Operator=\"$opr1\" Value=\"\" <br>";
			$CriteriaRow .= "<Criteria Row=\"$row\" Field=\"Appointment_Date\" Operator=\"$opr1\" Value=\"\" />";
			//$JoinOperator = "$row";
			if ($JoinOperator == '') {
				$JoinOperator = "$row";
			} else {
		...
		}else if($mode == 'InProgess'){
			$opr1 = "Equal"; 
			$row++;
			$CriteriaRow1 .= "Criteria Row=\"$row\" Field=\"Sold_Lost\" Operator=\"$opr1\" Value=\"In Progess\" <br>";
			$CriteriaRow .= "<Criteria Row=\"$row\" Field=\"Sold_Lost\" Operator=\"$opr1\" Value=\"In Progess\" />";
			//$JoinOperator = "$row";
			if ($JoinOperator == '') {
				$JoinOperator = "$row";
			} else {
	...
	//echo 'JoinOperator = ['.$JoinOperator.']..............................CriteriaRow1 = ['.$CriteriaRow.']';
...
function loadContactReportFilter($userTicket,$folder,$searchData,$mode) {
	$Contact = "";
	$rows = array();
	$FieldNames = array("FirstName","LastName","Phone","Mobile","Email","FollowUp","Followup_Date","Appointment_Date","SalesmanCode","Sold_Lost","Notes","PURL");
	//echo "userTicket=$userTicket,searchData=$searchData,LastName=$LastName,opr1=$opr1,opr2=$opr2<br>";
...
/*
	...
	$FieldHeader = $FieldNames; 
	$FieldHeader = str_replace("Followup_Date","Follow Up Date",$FieldHeader);
	$FieldHeader = str_replace("SalesmanCode","Sales Person",$FieldHeader);
	$FieldHeader = str_replace("Sold_Lost","Sale Status",$FieldHeader);
*/ 
...
function loadContactArrayFilters($email,$userTicket,$folder,$account,$jsonFilters) {	
	...
	echo "<br>FieldHeader:<br>";
	var_dump($FieldHeader);
	echo "<br>FieldNames:<br>";
	var_dump($FieldNames);
	echo "<br><br>";
